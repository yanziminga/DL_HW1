...
# model 3 using parameters from model 1 and model 2
tf.reset_default_graph()
x = tf.placeholder(tf.float32, shape=[None, img_flatten], name='x')
input_x = tf.reshape(x, [-1, img_size, img_size, 1])
y = tf.placeholder(tf.float32, shape=[None, num_classes], name='y')
...
for alpha in np.arange(-1, 2, 0.05):
    ...
    train_loss, train_acc = session.run([m3_loss, m3_acc_op],
                                        feed_dict={x: data.train.images, y: data.train.labels, m3_w_conv1: w_conv1,
                                                   m3_w_conv2: w_conv2, m3_w_fc1: w_fc1, m3_w_fc_out: w_fc_out,
                                                   m3_b_conv1: b_conv1, m3_b_conv2: b_conv2, m3_b_fc1: b_fc1,
                                                   m3_b_fc_out: b_fc_out})
    test_loss, test_acc = session.run([m3_loss, m3_acc_op],
                                      feed_dict={x: data.test.images, y: data.test.labels, m3_w_conv1: w_conv1,
                                                 m3_w_conv2: w_conv2, m3_w_fc1: w_fc1, m3_w_fc_out: w_fc_out,
                                                 m3_b_conv1: b_conv1, m3_b_conv2: b_conv2, m3_b_fc1: b_fc1,
                                                 m3_b_fc_out: b_fc_out})
...
fig.suptitle('Batch Size: 64 vs 256')
...
#### training with different learning rate:0.01 and 0.00001
### Model1 Architecture:learning rate-0.01
tf.reset_default_graph()
x = tf.placeholder(tf.float32, shape=[None, img_flatten], name='x')
input_x = tf.reshape(x,[-1,img_size,img_size,1])
y = tf.placeholder(tf.float32, shape=[None, num_classes], name='y')
...
### Model2 Architecture:learning rate-0.00001
tf.reset_default_graph()
x = tf.placeholder(tf.float32, shape=[None, img_flatten], name='x')
input_x = tf.reshape(x,[-1,img_size,img_size,1])
y = tf.placeholder(tf.float32, shape=[None, num_classes], name='y')
...
# result visualize
fig,axs1 = plt.subplots()
fig.suptitle('Learning Rate: 1e-2 vs 1e-5')
alpha_list = np.arange(-1,2,0.05)
axs1.plot(alpha_list,test_acc_list,'r')
axs1.plot(alpha_list,train_acc_list,'r--')
axs1.set_ylabel('Accuracy')
axs1.yaxis.label.set_color('red')
axs1.legend(['test','train'],loc=5)
